...
class AdalineMultiOutput:
    ...
    def predict_batch(self, X):
        ...
        """
        X_bias = np.insert(
            X, 0, 1, axis=1)  # Insertar el término de bias en todas las muestras
        # Producto punto para todas las salidas
...
X = np.array([noisy_composite_signal[i:i + delay]
             for i in range(n_samples - delay)])
print(X.shape)
# Señales originales retrasadas para alinear con X
d1 = signal1[delay:]
d2 = signal2[delay:]
d3 = signal3[delay:]
# Matriz de salidas deseadas (n_samples - delay, 3)
D = np.stack((d1, d2, d3), axis=1)

# Crear y entrenar la red ADALINE multisalida
adaline = AdalineMultiOutput(
    input_size=delay, output_size=3, learning_rate=0.001, epochs=200)

adaline = AdalineMultiOutput(
    input_size=delay, output_size=3, learning_rate=0.01, epochs=200)
adaline.train(X, D)

# Recuperar las señales utilizando la red ADALINE entrenada
# Matriz de salidas recuperadas (n_samples - delay, 3)
...
plt.subplot(4, 1, 2)
plt.plot(t[delay:], recovered_signal1,
         label="Señal Recuperada 1 (ADALINE)", color="r")
...
plt.subplot(4, 1, 3)
plt.plot(t[delay:], recovered_signal2,
         label="Señal Recuperada 2 (ADALINE)", color="g")
...
plt.subplot(4, 1, 4)
plt.plot(t[delay:], recovered_signal3,
         label="Señal Recuperada 3 (ADALINE)", color="b")
